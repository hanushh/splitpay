-- Fix add_group_members_by_ids: group_balances PK is now (group_id, user_id,
-- currency_code) after the multi-currency migration, but the function still
-- used ON CONFLICT (group_id, user_id) which no longer matches any constraint.
...
begin
  ...
  -- Use the 3-column PK introduced by the multi-currency migration.
  insert into public.group_balances (group_id, user_id, currency_code, balance_cents)
  select p_group_id, uid, 'INR', 0
