-- Add settlement push notification to record_settlement.
--
-- When a user records a settlement, the payee (the person receiving money)
-- gets a user_notifications row so the client can dispatch it immediately
-- via dispatchPendingPushNotifications() in settle-up.tsx.
...
BEGIN
  ...
  -- Notify the payee that they received a payment (skip if payee == payer, which
  -- can't happen logically but guard anyway)
