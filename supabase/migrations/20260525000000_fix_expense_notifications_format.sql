-- Fix: replace invalid %.2f printf-style specifier with PostgreSQL-compatible
-- to_char() so update_expense_with_splits and delete_expense no longer throw
-- "unrecognized format() type specifier" when notifying users.
  ...
  LOOP
    INSERT INTO public.user_notifications (
      ...
      format(
        ...
        to_char(v_expense.amount_cents / 100.0, 'FM999999990.00')
      ),
      jsonb_build_object(
